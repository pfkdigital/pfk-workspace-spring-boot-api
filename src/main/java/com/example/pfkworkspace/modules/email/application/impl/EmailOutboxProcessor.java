...
public class EmailOutboxProcessor {
    ...
    public void process() {
        ...
        for (EmailOutbox outbox : pending) {
            try {
                ...
                log.info("Email sent: id={} type={} recipient={}", outbox.getId(), outbox.getEmailType(), outbox.getRecipient());
            } catch (Exception e) {
                ...
                if (attempts >= MAX_ATTEMPTS) {
                    ...
                    log.error("Email id={} type={} permanently failed after {} attempts: {}", outbox.getId(), outbox.getEmailType(), attempts, e.getMessage());
                } else {
                    log.warn("Email id={} type={} failed, attempt {}/{}: {}", outbox.getId(), outbox.getEmailType(), attempts, MAX_ATTEMPTS, e.getMessage());
                }
            }
    ...
    private void dispatch(EmailOutbox outbox) {
        ...
        switch (outbox.getEmailType()) {
            case VERIFICATION -> emailService.sendVerificationEmail(outbox.getRecipient(), payload.get("token"));
            case ACCOUNT_VERIFIED -> emailService.sendAccountVerifiedEmail(outbox.getRecipient());
            case PASSWORD_RESET -> emailService.sendPasswordResetEmail(outbox.getRecipient(), payload.get("token"));
            case PASSWORD_UPDATED -> emailService.sendPasswordUpdatedEmail(outbox.getRecipient());
            case WORKSPACE_INVITATION -> emailService.sendWorkspaceInvitationEmail(outbox.getRecipient(), payload);
        }
    }
}
