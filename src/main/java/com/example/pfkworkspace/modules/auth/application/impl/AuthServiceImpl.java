...
public class AuthServiceImpl implements AuthService {
    ...
    public RegisterResponseDto register(RegisterRequestDto registrationRequestDto) {
        ...
        if (userRepository.existsByUsername(registrationRequestDto.username())) {
            throw new UsernameAlreadyExistsException("An account with this username already exists");
        }
        ...
        emailOutboxService.queue(registrationRequestDto.email(), EmailType.VERIFICATION, Map.of("token", rawToken));
    ...
    public VerifyResponseDto verify(String token) {
        ...
        emailOutboxService.queue(user.getEmail(), EmailType.ACCOUNT_VERIFIED);

        log.info("Email verified for userId={}", user.getId());
        return VerifyResponseDto.builder()
                .message("Email verified successfully. Your account is now active. Please log in to continue.")
                .build();
    }
    ...
    public CurrentUserResponseDto getCurrentUser() {
        ...
        if (user == null) {
            throw new AuthenticationCredentialsNotFoundException("Authenticated user not found in database");
        }
    ...
    public ForgotPasswordResponseDto forgotPassword(ForgotPasswordRequestDto forgotPasswordRequestDto) {
        final String genericMessage = "If an account with that email exists, a password reset link has been sent.";

        return userRepository.findByEmail(forgotPasswordRequestDto.email())
                .map(user -> {
                    ...
                    emailOutboxService.queue(forgotPasswordRequestDto.email(), EmailType.PASSWORD_RESET, Map.of("token", rawToken));
    ...
    @Override
    @Transactional
    public UpdatePasswordResponseDto updatePassword(UpdatePasswordRequestDto updatePasswordRequestDto, String token, HttpServletResponse response) {
        String tokenHash = sha256Hash(token);
        ...
        emailOutboxService.queue(user.getEmail(), EmailType.PASSWORD_UPDATED);
