...
public class WorkspaceInvitationImpl implements WorkspaceInvitationService {
    ...
    @Override
    @Transactional
    public InvitationResponseDto addMemberToWorkspace(CreateInvitationRequestDto createInvitationRequestDto, UUID workspaceId) {
        if (!workspaceSecurityService.isOwnerOrAdmin(workspaceId)) {
            throw new AuthorizationDeniedException("You must be a user or admin of the workspace to add a new member");
        }

        User derivedUser = userRepository.findByEmail(createInvitationRequestDto.email())
                .orElseThrow(() -> new UserNotFoundException("User not found with the email " + createInvitationRequestDto.email()));

        Workspace selectedWorkspace = workspaceRepository.findById(workspaceId)
                .orElseThrow(() -> new WorkspaceNotFoundException("Workspace not found with the id: " + workspaceId));
        ...
        emailOutboxService.queue(derivedUser.getEmail(), EmailType.WORKSPACE_INVITATION,
                buildInvitationPayload(workspaceId, selectedWorkspace.getName(), createInvitationRequestDto.role(), expiryDate, token));
    ...
    public void acceptInvitation(String token) {
        ...
        User newUser = userRepository.findByEmail(newUserEmail)
                .orElseThrow(() -> new UserNotFoundException("User not found with email: " + newUserEmail));
    ...
    public void declineInvitation(String token) {
        ...
        if (currentUser.getEmail() == null || !currentUser.getEmail().equals(workspaceInvitation.getEmail())) {
            throw new AuthorizationDeniedException("You are not authorized to reject this invitation, please use the email that the invitation was sent to");
        }
    ...
    public void resendInvitation(UUID invitationId) {
        WorkspaceInvitation workspaceInvitation = workspaceInvitationRepository.findById(invitationId)
                .orElseThrow(() -> new WorkspaceInvitationNotFoundException("Workspace Invitation not found for id: " + invitationId));
        ...
        if (!workspaceInvitation.getIsUsed() && workspaceInvitation.getExpiresAt().isAfter(Instant.now())) {
            throw new RuntimeException("Workspace Invitation is still valid, cannot resend the invitation");
        }
        ...
        emailOutboxService.queue(workspaceInvitation.getEmail(), EmailType.WORKSPACE_INVITATION,
    ...
    private Map<String, String> buildInvitationPayload(UUID workspaceId, String workspaceName, WorkspaceRole role, Instant expiryDate, String token) {
        return Map.of(
    ...
    private WorkspaceInvitation getAndValidateInvitationForCurrentUser(String token) {
        ...
        WorkspaceInvitation workspaceInvitation = workspaceInvitationRepository.findByTokenHash(tokenHash)
                .orElseThrow(() -> new WorkspaceInvitationNotFoundException("Workspace Invitation not found for token: " + token));

        if (currentUser.getEmail() == null || !currentUser.getEmail().equals(workspaceInvitation.getEmail())) {
            throw new AuthorizationDeniedException("You are not authorized to accept this invitation, please use the email that the invitation was sent to");
        }
